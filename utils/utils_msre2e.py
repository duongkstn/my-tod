...
def read_langs_turn(args, file_name, max_line=None, ds_name=""):
    ...
    with open(file_name) as f:
        ...
        for dial in dials[1:]:
            ...
            need_translate.append(Message.strip())
...
def prepare_data_msre2e(args):
    ...
    pair_mov, need_translate_mov = globals()["read_langs_{}".format(_example_type)](args, file_mov, max_line, ds_name + "-mov")
    pair_rst, need_translate_rst = globals()["read_langs_{}".format(_example_type)](args, file_rst, max_line, ds_name + "-rst")
    pair_tax, need_translate_tax = globals()["read_langs_{}".format(_example_type)](args, file_tax, max_line, ds_name + "-tax")

    pair_trn = pair_mov + pair_rst + pair_tax
    pair_dev = []
    pair_tst = []
    import pandas as pd
    print(len(need_translate_tax), len(need_translate_rst), len(need_translate_mov))
    need_translate = list(set(need_translate_rst + need_translate_tax + need_translate_mov))
    need_translate = sorted(need_translate)
    pd.DataFrame({'text': need_translate}).to_excel("/media/user/Data/ToD-BERT/TODBERT_dialog_datasets/dialog_datasets/Translated/train_dumpdata_tod_msre2e.xlsx", index=False)
    exit(0)
