...
def read_langs_turn(args, file_names, ontology, dialog_act, max_line=None, domain_act_flag=False, update_ont_flag=False):
    print(("Reading from {} for read_langs_turn".format(file_names)))
    ...
    add_slot_values = set()
    df = pd.read_excel(
        os.path.join(args["data_path"], "Translated", "train_dumpdata_tod_multiwoz_translated_tokenized.xlsx"),
        dtype=str,
    )
    ...
    for file_name in file_names:
        ...
        with open(file_name) as f:
            ...
            for dial_dict in dials:
                ...
                number_of_turns += len(dial_dict["dialogue"])
                for ti, turn in enumerate(dial_dict["dialogue"]):
                    ...
                    if turn["turn_idx"] == 0 and turn["system_transcript"] == "":
                        ...
                        dialog_act[dial_dict["dialogue_idx"].replace(".json", "")][str(turn["turn_idx"])] == "No Annotation"
                    ):
                        cur_sys_acts = set()
                    else:
                    ...
                    # if data_detail["turn_usr"] not in dict_translated:
                    #     print("turn = ", data_detail["turn_usr"])
                    data_detail["turn_usr"] = dict_translated.get(data_detail["turn_usr"].strip(), data_detail["turn_usr"].strip()).strip()
                    # if data_detail["turn_sys"] not in dict_translated:
                    #     print("turn = ", data_detail["turn_sys"])
                    data_detail["turn_sys"] = dict_translated.get(data_detail["turn_sys"].strip(), data_detail["turn_sys"].strip()).strip()
...
def prepare_data_multiwoz(args):
    ...
    file_trn = [os.path.join(args["data_path"], "MultiWOZ-{}/train_dials1.json".format(version)),
                os.path.join(args["data_path"], "MultiWOZ-{}/train_dials2.json".format(version))]
